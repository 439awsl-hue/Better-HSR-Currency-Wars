...
public sealed class AutomationRuntime
{
	...
	public AutomationRuntime(AutomationConfig config, Func<FlowStep, CancellationToken, Task> executeStepAsync, Func<double, CancellationToken, Task> delayAsync, Func<double, CancellationToken, Task> variableDelayAsync, Func<string, Task> logAsync)
	{
		_config = config;
	...
	public async Task RunAsync(CancellationToken cancellationToken)
	{
		...
		while (!cancellationToken.IsCancellationRequested)
		{
			...
			foreach (FlowStep step in CurrencyWarsFlow.Steps)
			{
				...
				if (step.StandardDelayAfterSeconds > 0.0)
				{
